...
const entries = () => {
  ...
  const entriesFile = glob.sync(path.resolve(__dirname, '../client/entries/*.ts'))
  /**
   * 入口字典
   * {
...
const config = {
  ...
  mode: 'development',
  output: {
    path: path.resolve(__dirname, isDev ? '../dist/' : '../built'),
  ...
  module: {
    rules: [{
      ...
      include: [
        ...
        path.resolve(__dirname, '../client/entries')
      ],
      use: [{
        loader: 'ts-loader',
        options: {
...
Object.keys(config.entry).forEach(entry => {
  config.plugins.push(new HtmlWebpackPlugin({
    ...
    template: path.resolve(__dirname, `../client/views/${entry}.html`),
    chunks: ['app', entry],
